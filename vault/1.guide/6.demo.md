...
A standalone `ThreeColumnDemoSceneView` demonstrates the optional content column available in `KelyphosShellConfiguration`:
...
The `content:` builder adds a middle column between the navigator and the detail area — suitable for master/detail/detail layouts like mail, notes, or document browsers.
